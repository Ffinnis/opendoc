...
enum NativeAgentHelp {
    ...
    static func command(_ name: String) throws -> [String: Any] {
        ...
        case "widget.preview":
            ...
            result = "result contains value, detail, and optional progress (0...1). The render function may also return style (ring/bar), tint (level/blue/purple/pink/red/orange/yellow/green/teal/indigo/gray; level turns green, yellow, then red as progress falls), symbol (an SF Symbol name, also used as the tile's glyph), and apps (up to 4 bundle identifiers; the first available app's icon is shown, never launched; Claude falls back to the installed CodexBar logo when Claude Desktop is absent). Unsupported presentation values are ignored rather than failing the widget. Code receives state, input.text, input.matches. It has no file/shell/network APIs and stops after 2 seconds. HTML extraction reads returned HTML without site scripts or browser login."
        case "dock.update":
